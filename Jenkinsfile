...
stages {
  stage(' Docker Build'){ // docker build image stage
    steps {
      ...
      sh '''
        ...
        # NEW: also tag "latest" locally so we can publish it after tests
  ...
  stage('Docker run'){ // run container from our builded image
    steps {
      ...
      sh '''
      docker rm -f jenkins || true
      # NEW: run the just-built versioned image locally (no registry dependency here)
    ...
    steps {
      script {
        sh '''
          ...
          # NEW: publish "latest" too, so external runs can always pull the newest image
